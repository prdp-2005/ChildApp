...
const Plane: React.FC<PlaneProps> = ({ item, slug, selectedIndex, onClick, mounted }) => {
    return (
        <div
            ...
            className={`flex flex-col items-start justify-center gap-5 w-80 p-6 border rounded-[20px] cursor-pointer shadow-[34.854px_29.626px_48.34px_rgba(51,102,255,0.05)] ${selectedIndex === item.id
                ? "bg-[#1DF2F2] border-[1px] border-black text-black"
                : "dark:bg-dark"
                }`}
        >
            ...
            <div className="flex items-baseline gap-2">
                <p className={`text-4xl lg:text-5xl font-bold ${selectedIndex === item.id ? "text-black" : "text-gray-800 dark:text-white"}`}>
                    ...
                    {/* {slug === 'en' ? (mounted ? "$8.32" : "$14.90") : (mounted ? "8.32€" : "14.90€")} */}
                </p>
            ...
            <div className="flex flex-col gap-1">
                <p className={`flex flex-row gap-2 items-center font-bold text-nowrap ${selectedIndex === item.id ? "text-black" : "text-[#2D2D2D] dark:text-white"}`}>
                    <FaCheck className={selectedIndex === item.id ? "bg-black" : ""} color="#1DF2F2" />
                    {/* {slug === 'en' ? "Write feature details here" : "Fügen Sie hier Feature-Details ein"} */}
                    {item.f1}
                </p>
                <p className={`flex flex-row gap-2 items-center font-bold ${selectedIndex === item.id ? "text-black" : "text-[#2D2D2D] dark:text-white"}`}>
                    <FaCheck className={selectedIndex === item.id ? "bg-black" : ""} color="#1DF2F2" />
                    {item.f2}
                </p>
                <p className={`flex flex-row gap-2 items-center font-bold ${selectedIndex === item.id ? "text-black" : "text-[#2D2D2D] dark:text-white"}`}>
            ...
            <div className={`py-4 px-12 flex justify-center items-center border border-[#0E121B] rounded-[20px] w-full ${selectedIndex === item.id ? "bg-white text-black" : "dark:bg-white dark:text-black"}`}>
                <p className="flex flex-row gap-2 items-center bg-transparent">
                    {slug === 'en' ? "Get Started" : "Loslegen"}
                    <FaArrowRightLong className="hover:text-black" />
                </p>
